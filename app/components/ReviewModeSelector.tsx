...
export const ReviewModeSelector = ({ selectedModes, onModeChange }: ReviewModeSelectorProps) => {
  ...
  const handleKeyDown = (e: any, modeValue: string, group: ReviewModeGroup, isChecked: boolean, isDisabled: boolean) => {
            ...
            {group.modes.map((mode) => {
              ...
              return (
                <div key={mode.value} className="relative group">
                  <label
                    className={`flex items-start w-full p-3 rounded-md transition-colors border ${isChecked
                  ...
                  <div
                    ...
                    role="tooltip"
                    aria-live="polite"
                    className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs p-2 text-xs text-white bg-gray-900 border border-gray-600 rounded-md shadow-lg transition-opacity duration-300 z-10 ${showTooltip ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 pointer-events-none'
                      }`}
                  >
                    {mode.description}
